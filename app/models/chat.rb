class Chat < ApplicationRecord
  ...
  # get chats which updated in the last 45 minutes and update it's message_count to the current number of messages in the chat
  def self.update_messages_counter
    chats = Chat.where("updated_at >= ?", Time.now - 45.minutes)
    chats.each do |chat|
